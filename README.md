...
- **Heartbeat и last_seen**: текущая реализация при вызове `PATCH /heartbeat` обновляет только поле `is_online`, но не меняет `last_seen`. Это означает, что фоновая задача `Killer` (запускается с периодом 30 секунд) будет помечать пир как офлайн, если с момента его создания прошло больше периода неактивности, независимо от heartbeat. Для корректного продления жизни пира необходимо в методе `SetOnline` также обновлять `peer.LastSeen = time.Now()`.
